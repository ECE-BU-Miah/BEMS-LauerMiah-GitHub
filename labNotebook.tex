...
\date{Beginning March 06, 2019} % Beginning date
\maketitle
...
I read up on the linux command \texttt{readlink} that I discovered in the BEMOSS installation script in the BEMOSS$3.5$/GUI directory. It "prints the value of a symbolic link or canonical file name". If the command \texttt{readlink -f "\$0"}
is run in a shell script it prints the full path to the shell script or alternatively it can be run on the command link which prints the full path to a provided file. The objective is to determine how to connect to the meta database which lies in the WebServer directory from any file in the codebase. 
\medbreak\noindent
Instead of using the \texttt{readlink} command, I decided to use the python function \texttt{os.getcwd()} from the os module as it is a much simpler way of returning the working directory of a file. I then created a file named \texttt{global\_settings.py} in the base project dir which holds global string variables containing paths to all the project directories. These path constants were subsituted for the full paths provided to some of the \texttt{sqlite3.connect} calls.
\medbreak\noindent
Next, what I would like to do is to create an installation shell script to allow anyone to clone the repository and succesfully install the current working version of the software from scratch on an Ubuntu laptop/desktop. Here's what the shell script will do 
\begin{enumerate}
...
\end{enumerate}
To prevent the \texttt{TempTest/} and \texttt{venv/} directories and all \texttt{\_\_pycache\_\_} directories from being added, I created a \texttt{.gitignore} file with the following contents:
\begin{Verbatim}
# Compiled python
__pycache__/
.pyc
...
venv/
\end{Verbatim}
However, since I am kind of still new to git, the unwanted files/directories placed in the \texttt{.gitignore} will not go away once the changes are pushed to the master branch. The command
\begin{verbatim}
git rm -rf --cached .
\end{verbatim}
will unstage and remove paths from the index but keep the files in the working tree on the local disk \footnote{\url{https://stackoverflow.com/questions/25436312/gitignore-not-working/25436481}}. A problem was encountered when running the \texttt{run.sh} script as I made a syntax error in an if statement.
\begin{Verbatim}
./run.sh: line 17: [: missing `]'
\end{Verbatim}
this code is found in the section of the script where I add the database file named \texttt{meta.db} which is shown below:
\begin{Verbatim}
# Setup the db file
cd $webserver_dir
if [ -f "meta.db"]; then
...
# $
\end{Verbatim}
After some Googling I found that I need a space between "meta.db" and the "]" character. I was able to succesfully fix all the syntax errors and bugs, so now I will try to test the installation of the software on a Raspberry Pi although a VM would be perfectly acceptable as well.

\labday{Thursday, June 25, 2020}
Today, I copied the platform files into the \texttt{BEMS-LauerMiah-GitHub} directory into the directory \texttt{code}. When I run the script to run the platform, I found that the directory \texttt{code} interferes with a python module in the standard library called \texttt{code} which enables the ability to emulate Python's interactive interpreter. One of the \texttt{werkzeug} libraries was throwing an exception when it tried to interpret this module as the root directory of the project was named the same. After renaming the directory to \texttt{NewBEMS} everything appeared to work fine. Also, I changed the operator '>' to '>>' in the \texttt{install.sh} script to append to the .pth files rather than over write.

\labday{Friday, June 26, 2020}
The tasks for today are the following:
\begin{itemize}
\item Implement a feature to allow the Device Discovery agent to loop a set number of times and call the find\_devices function for each supported device API module. 
\end{itemize}
I decided that the modal popup is not really necessary if auto discovery will be enabled by default as the devices will simply show up on the page and the user will not need to be notified of this through a modal. I problem I ran into is the fact that the web pages are not uploading despite removing the modal button and popup being removed from the \texttt{discovery.html} page. It was found that I was running the server from the wrong directory as I have the software installed in my home directory. 

\labday{Tuesday, June 30, 2020}
The problem I was having the problem of svgs not rendering in Chrome as I would like to add an icon made in inkscape for each device.

\labday{Wednesday, July 1, 2020}
After changing a line in the svg file created by inkscape, the image properly rendered in chrome. Today, I worked on reworking the discovery process by eliminating the search for device page and just having a single page for active devices. Also, I added some HTML to display a message when no devices are found on the network. I created a second python flask view function module \texttt{active\_devices.py} which will handle urls to the active\_devices page rather than the discovery page. I simplified the discovery process by having a single method titled \texttt{searchForDevices} in the \texttt{DiscoveryAgent} class which will simply perform the following actions:
\begin{itemize}
\item Pull apis from the \texttt{SupportedDevices} table
\item Import each API module corresponding to that API
\item Call findDevices on each API module and assign the url list to a variable
\item Retreive the metadata from each list
\item Add metadata to the \texttt{ActiveDevices} table
\item Send post request to the server to get the active devices from the table and render to the UI
\end{itemize}
The problem I am trying to solve now is how to get an update from the agent when it has completed discovering devices. What this will require is sending a message to the UI. A solution to this problem is by simply returning a tuple from a flask route~\footnote{\url{https://stackoverflow.com/questions/24295426/python-flask-intentional-empty-response}}. The response will be empty with a status of $204$ (no response). Even though it may not be the best implementation, I decided to add a global variable \texttt{done\_discovering} which will be set to true when a request has been sent to the url \texttt{/active\_devices/agents} by the DiscoveryAgent and the \texttt{renderActiveDevices} function will constantly wait for it to be set to True before rendering data to the html page.
\medbreak\noindent
A problem I am running into is the fact that the Discovery Agent is adding devices to the ActiveDevices table. However, when the following query is made by the flask backend:
\begin{Verbatim}
SELECT * FROM ActiveDevices;
\end{Verbatim}
it returns an empty list. Although changes were made shortly before it to add devices to the table. The mistake I made was forgetting to include the statement:
\begin{Verbatim}
conn.commit()
\end{Verbatim}
which commits the changes to the database after inserting into the \texttt{ActiveDevices} table.
\medbreak\noindent
The next task is to add a collapsible accordian to the \texttt{active\_devices} page to display all the devices that can be controlled in one page~\footnote{\url{https://getbootstrap.com/docs/4.3/components/collapse/}}. In addition, I added a column to the \texttt{ActiveDevices} table, image, which will contain a string specifiying an svg in the static/img directory.	

\labday{Monday, July 6, 2020}
What I worked on today was getting the on/off switch to work for the WeMo Switch. First, I added a method to the control agent titled \texttt{setDeviceStatus}. Since this method will be required to call the \texttt{setState} method of the device that requests it, an api column is required in the ActiveDevices table. Then, the module will be imported using the built in \texttt{importlib} module by calling \texttt{import\_module}. Since I realized that the url is required to be passed in to the \texttt{setState} function of the WeMoAPI, I decided to instead add two more fields to the active devices table titled \texttt{ip} and \texttt{port}. The url will parsed using \texttt{urllib.parse.urlparse}. Then different properties can be referenced from the returned object such as the \texttt{scheme},\texttt{netloc}, \texttt{path}, \texttt{parameters}, \texttt{query}, and \texttt{fragment}. Using the following code:
\begin{Verbatim}
...
\end{Verbatim}
the ip and port is obtained. I replaced \texttt{url} as the first argument to the \texttt{setState} function with \texttt{deviceId} which will be the id of the target device.

\labday{Tuesday, July 7, 2020}
I worked on getting Jquery code added to the active devices page by adding a file titled \texttt{active\_device.js} with the following function:
\begin{Verbatim}
$(document).ready(function() {
...
\end{Verbatim}
Essentially, when a device is pressed the corresponding id of the device is the id of the button element. Then, the state of the device is assigned to the key "powerState" whether it be "ON" or "OFF". This dictionary is sent with the powerState and id key/value pairs as a json encoded string to the server. However, the click function does not appear to be called when the toggle button is clicked. When the \texttt{data-toggle} attribute of the \texttt{input} element described below:
\begin{Verbatim}
<input type="checkbox" id="wemo" data-onstyle="success" data-toggle="toggle">
\end{Verbatim}
is removed, the checkbox no longer becomes a bootstrap toggle checkbox but rather a normal checkbox. This does solve the problem of the on click listener not working properly in Jquery though. Therefore, it must be determined how to properly listen for onclick events from the bootstrap toggle button.

\labday{Wednesday, July 8, 2020}
I was able to get the bootstrap toggle function to be detected with javascript using the \texttt{change} method:
\begin{Verbatim}
...
\end{Verbatim}
A function similar to the one above was written to send an AJAX request to the backend. 
\begin{Verbatim}
$('input[type="checkbox"]').change(function() {
	// evt.preventDefault();
...
\end{Verbatim}
A potential problem I see in using publish/subscribe in sending data to agents is the fact that the following exception is thrown if the toggle button is pressed too frequently:
\begin{Verbatim}
zmq.error.ZMQError: Address already in use
\end{Verbatim}
as multiple publishes are being attempted using the same address. The line
\begin{Verbatim}
python3 ControlAgent.py &
\end{Verbatim}
which will run the Control Agent. However, a problem exists in the \texttt{subscribe} method as the message received from the web server:
\begin{Verbatim}
self.msg = socket.recv().decode('utf-8')
\end{Verbatim}
does not unpack to three different strings when using the line
\begin{Verbatim}
topic, method, args = self.msg.split()
\end{Verbatim}
as \texttt{args} is a dictionary which contains a space.
\end{addmargin}
